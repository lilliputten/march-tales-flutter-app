...
# Create a signed bundle (aab)
if [[ "$ARGS" =~ .*--aab.* ]]; then
  AAB_FOLDER="build/app/outputs/bundle/release"
  echo "Generating an aab bundle $PROJECT_INFO #${VERSION_CODE}..."
  # Old debug info path (gradle 8.3):
  # DEBUG_INFO_PATH="build/app/intermediates/merged_native_libs/release/out/lib"
  # New debug info path (gradle 8.7):
  DEBUG_INFO_PATH="build/app/intermediates/merged_native_libs/release/mergeReleaseNativeLibs/out/lib"
...
if [[ "$ARGS" =~ .*--apk.* ]]; then
  ...
  echo "Generating an apk build $PROJECT_INFO #${VERSION_CODE}..."
  flutter build apk \
    --build-name=$VERSION \
    --dart-define-from-file=.env \
    && echo "See the apk in the $APK_FOLDER" \
