...
class PlayerBoxState extends State<PlayerBox> {
  ...
  void _savePlayingPosition(Duration? position, {bool notify = true}) {
    ...
    // XXX -- 2025.03.01, 21:06 -- Update position on the server
    // XXX Involve last saved position and save once in a period, eg, 5 secs
  }

  Future<void> _loadTrackPosition() async {
    if (this._track != null) {
      // XXX Load data from the server if isAuthorized
